...
Atlas "themes" allow the author or designer to style the various output formats using CSS.  This is important because reflowable formats like ebooks and websites have very different design constraints  than fixed formats like PDF or print.  Consequently, it's important that designers be able to easily create different styles that are consistent across the formats. Atlas has two default themes: a trade template and a technical template.  
